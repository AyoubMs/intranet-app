...
let selectItem = (itemData, type) => {
    switch (type) {
        case 'team':
            teamStore.selectTeam(itemData)
    }
}
    ...
    <div class="mb-6 w-1/2 mx-2">
        ...
        <div class="w-full flex flex-col items-center mx-auto justify-end"
                        ...
                        <div class="flex flex-auto flex-wrap">
                            ...
                            <div class="flex-1">
                                <input :type="type" :name="val" :id="val"
                                       class="bg-transparent p-1 px-2 appearance-none outline-none h-full w-full text-gray-800 rounded !border-transparent !ring-transparent" @click="openDropDown = true" @focusout="closeDropDown()" v-model="searchInput">
                            </div>
                        </div>
                        <div
            ...
            <div class="shadow bg-white z-40 h-40 w-full lef-0 rounded max-h-select overflow-y-auto svelte-5uyqqj"
                ...
                <div class="flex flex-col w-full" v-if="controlWithSearch">
                    <div v-for="(item, index) in data" :key="index"
                        ...
                        <div v-if="val === 'team'" v-for="user in item?.users"
                             ...
                             @click="selectItem([item?.operation, ` / ${user?.first_name} ${user?.last_name} ${user?.team_type !== null ? `/ ${user?.team_type}` : '/ Customer Service'}`], 'team')">
