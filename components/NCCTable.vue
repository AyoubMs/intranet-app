...
const showAcceptOrReject = (demand) => {
  ...
  if (!demand) {
    return isSupervisor || isOpsManager || isWfm || isRespIT || isChargeRH || isRespRH || isCoordinatorQualiteFormation
  }
  if (isCoordinatorQualiteFormation && demand) {
    return (created && isCoordinatorQualiteFormation && isChargeFormationFunc(demand?.user?.role?.name))
  }
  ...
  if (isHR && demand && demand?.user_id !== userId) {
    return (validatedByOpsManager && !isSupervisorFunc(demand?.user?.role?.name)) || (validatedByAgentWFM && isChargeRH && isSupervisorFunc(demand?.user?.role?.name)) || (validatedByCoordinatorWFM && isRespRH && isSupervisorFunc(demand?.user?.role?.name)) || (validatedByDirector && isWFMAgentFunc(demand?.user?.role?.name) && isRespRH) || (created && isCoordinatorWFM(demand?.user?.role?.name) && isRespRH) || (validatedByCoordinatorWFM && isChargeRH) || (created && isRespRH && isChargeRHFunc(demand?.user?.role?.name)) || (validatedByRespIT && isChargeRH) || (validatedByDirector && isRespRH && isRespITFunc(demand?.user?.role?.name)) || (created && isRespRH && isRespITFunc(demand?.user?.role?.name)) || (created && isRespRH && isDeveloperFunc(demand?.user?.role?.name)) || (created && isRespRH && isAgentMG(demand?.user?.role?.name)) || ((created || validatedByDirector) && isRespRH && isRespMG(demand?.user?.role?.name)) || (created && isRespRH && isInfirmiereDeTravailFunc(demand?.user?.role?.name)) || (validatedByDirector && isRespRH && isChargeMissionAupresDirection(demand?.user?.role?.name)) || (validatedByDirector && isChargeRH && isChargeCommMktgFunc(demand?.user?.role?.name)) || (validatedByCoordinatorQualiteFormation && isRespRH && isChargeFormationFunc(demand.user?.role?.name))
  }
...
<template>
  ...
  <table v-if="userStore.users?.data && !inputStore.loadingTable || type === 'demandesConges'"
    ...
    <tr class="text-center" v-for="(demand) in demandInputStore.demands?.data" v-if="type === 'demandesConges'">
      ...
      <Modal :show="showModal" :accept="accept" :reject="reject" :cancel="cancel" :demande-conge-data="selectedDemand"
             @close="closeModal()"/>
    </tr>
