...
function getSpecificLevelValidationConditionOrProchainValidateur(level: any, demandeCongeDataFromProps: any, type: string) {
  for (let profile in scenarios(demandeCongeDataFromProps)) {
    for (let scenarioKey in scenarios(demandeCongeDataFromProps)[profile]) {
      if (scenarios(demandeCongeDataFromProps)[profile][scenarioKey]?.niveau1?.condition && level === 1) {
        if (type === 'validationNumber') {
          return true;
        } else if (type === 'prochainValidateur') {
          return scenarios(demandeCongeDataFromProps)[profile][scenarioKey].niveau2?.role ?? ""
        }
      } else if (scenarios(demandeCongeDataFromProps)[profile][scenarioKey]?.niveau2?.condition && level === 2) {
        if (type === 'validationNumber') {
          return true;
        } else if (type === 'prochainValidateur') {
          return scenarios(demandeCongeDataFromProps)[profile][scenarioKey].niveau3?.role ?? ""
        }
      } else if (scenarios(demandeCongeDataFromProps)[profile][scenarioKey]?.niveau3?.condition && level === 3) {
        if (type === 'validationNumber') {
          return true;
        } else if (type === 'prochainValidateur') {
          return scenarios(demandeCongeDataFromProps)[profile][scenarioKey].niveau4?.role ?? ""
        }
...
function getDateValidationNumberOrProchainValidateur(demandeCongeDataFromProps: any, type: string) {
  if (getSpecificLevelValidationConditionOrProchainValidateur(1, demandeCongeDataFromProps, type)) {
    if (type === 'validationNumber') {
      return 1;
    } else if (type === "prochainValidateur") {
      return getSpecificLevelValidationConditionOrProchainValidateur(1, demandeCongeDataFromProps, type)
    }
  } else if (getSpecificLevelValidationConditionOrProchainValidateur(2, demandeCongeDataFromProps, type)) {
    if (type === 'validationNumber') {
      return 2;
    } else if (type === "prochainValidateur") {
      return getSpecificLevelValidationConditionOrProchainValidateur(2, demandeCongeDataFromProps, type)
    }
  } else if (getSpecificLevelValidationConditionOrProchainValidateur(3, demandeCongeDataFromProps, type)) {
    if (type === 'validationNumber') {
      return 3;
    } else if (type === "prochainValidateur") {
      return getSpecificLevelValidationConditionOrProchainValidateur(3, demandeCongeDataFromProps, type)
    }
  } else if (getSpecificLevelValidationConditionOrProchainValidateur(4, demandeCongeDataFromProps, type)) {
    if (type === 'validationNumber') {
      return 4;
    } else if (type === "prochainValidateur") {
      return getSpecificLevelValidationConditionOrProchainValidateur(4, demandeCongeDataFromProps, type)
    }
  }
}
...
const sendAcceptationDataToBackend = async () => {
  ...
  let dataObj = {
    ...
    date_validation_number: getDateValidationNumberOrProchainValidateur(props.demandeCongeData, 'validationNumber') ?? 0,
    nombre_jours_confirmed: nbrJrsDemandesConfirmed.value,
    prochain_validateur: getDateValidationNumberOrProchainValidateur(props.demandeCongeData, 'prochainValidateur') ?? ""
  };
